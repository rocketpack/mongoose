...
var Document = function(){
  ...
  Object.defineProperty(this, '_', {value: {
    ...
    errors: [],
    fields: arguments[2] // todo fails if callback is pass as well.
  }});
...
Document.prototype._run = function(name, fn, args){
  ...
  var total = pres.length
    ...
    , process = function(err){
        if(err) this._.errors.push(err);
        if(++current >= total ) complete.apply(self);
        else pres[current].call(self, process, complete);
      }
    , complete = function(){
        current = total;
  ...
  if(posts.length){
    callback = (function(n, cb){
        return function(){
          ...
          if(name != 'save' || self._.errors.length == 0) 
            for(var i=0, l=posts.length; i<l; i++) posts[i].call(self);
        }
    })(name, callback);
  }
  ...
  if(name == 'save'){
    this._validate(override, process, args);
  } else { 
    process();
  }
};
...
Document.prototype.set = function (path, val, override) {
  ...
  if (override) {
    ...
    for (i=setters.length-1; i>=0; i--) {
      try {
        var ret = setters[i].call(this, val, path, override);
        if (Error == ret) {
    ...
    if (typeof type._castSet === "function") val = type._castSet.call(this, val, path, override);
