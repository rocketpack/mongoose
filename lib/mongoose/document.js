...
Document.prototype._setData = function(struct, obj, val, path, override){
  ...
  for(i=0,l=struct.length; i<l; i++){
    ...
    if(typeof prop == 'string'){
      ...
      } else if(override && undefined !== schema.paths[curpath]._default && this.loadedField(curpath)){ // set defaults 
...
Document.prototype._run = function(name, fn, args){
  ...
  var total = pres.length,
      process = function(err){
        ...
        else pres[total](process, complete);
      },
      complete = function(){
        total = 0;
...
module.exports = {
  ...
  Hooks: {
    ...
    save: function(fn){
        ...
        if(this.isNew){
          this._collection.save(this._.doc, {safe: true}, function(err){
            self.isNew = false;
            if(fn) fn(err, self);
          });
        } else {
          this._collection.update({_id: this._id},{$set: this._getDirty()}, {safe: true}, function(){
            self._dirty = {};
            if(fn) fn(err, self);
          });
        }
  ...
  // self._schema._indexes.push(collectionInfo[indexName]);
          }
          fn(_indexes);
        });
      }
