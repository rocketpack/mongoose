...
Document.prototype._run = function(name, fn, args){
  ...
  if(name == 'save'){
    ...
    for(var i=0, l = compilers.length; i < l; i++){
      ...
      if(this.isDirty(path)) type._compiler.call(this, path, type);
    }
 ...
 */
Document.prototype._get = function(path, obj){
...
Document.prototype.get = function(path){
  ...
  if(type.type == 'object') return val;
  
  var subtype = type.subtype
    , val = this._get(path);
  
  if(typeof type._castGet == 'function'){
    val = type._castGet.call(this, val, path, type);
  }
  for(var i=0, l=type.getters.length; i<l; i++){
    val = type.getters[i].call(this, val, path, type);
  }
  return val;
};
...
var EmbeddedArray = function(arr, path, subtype, scope, hydrate){
    ...
    Object.defineProperty(this, '_', {
      value: {
        ...
        stack: [] // queues changes to the array, used for compile.
      }, enumerable: false
    });
    ...
    Object.defineProperty(this, '_dirty', { value: false, enumerable: false });
    Object.defineProperty(this, '_stack', { value: [], enumerable: false });
...
EmbeddedArray.prototype =  {
  ...
  set: function(idx, val){
    this.arr[idx] = (this._.doc) ? new this._.doc(val) : val;
    this._stack.push(['set', this.arr[idx]]);
    this._dirty = true;
    return this;
  },
  ...
  push: function(item){
    for(var i=0,l=arguments.length; i<l; i++){
      var val = (this._.doc) ? new this._.doc(item) : item;
      var idx = this.arr.push( val ) //- 1;
      this._stack.push(['push', this.arr[idx-1]]);
      this._dirty = true;
    }
    return this;
  },
  ...
  shift: function(){ // removes first item
    ...
    this._stack.push(['shift', val]);
    this._dirty = true;
    return val;
  },
  
  unshift: function(item){ // adds to the beginning
    for(var i = 0, l = arguments.length; i < l; i++){
      this.arr.unshift((this._.doc) ? new this._.doc(item) : item);
      this._stack.push(['unshift', this.arr[0]]);
      this._dirty = true;
    }
  ...
  Hooks: {
    ...
    save: function(fn){
        ...
        if(this.isNew){
          ...
          var dirty = this._getDirty();
          this._collection.update(dirty.query, dirty.update, {safe: true}, function(err){
            self._dirty = {};
            if(fn) fn(err, self);
          });
        }
