    ...
    var promise = {
      do: function (fn) {
        if (this._.dbrefs[path]) { 
          if (this._.dbrefs[path]._id.toHexString() === this._.doc[path].oid.toHexString()) { 
            fn(null, this._.dbrefs[path]);
          } else {
            this._schema.paths[path].subtype.findById(this._.doc[path].oid, function (err, record) {
              fn(err, record);
            });
          }
