      ...
      do: function (fn) {
        if (this._.dbrefs[path]) {
          if (this._.dbrefs[path].id === this._.doc[path]['$id'].toHexString()) {
            ...
            this._schema.paths[path].subtype.findById(this._.doc[path]['$id'].toHexString(), function (err, record) {
              if (err) throw err;
              fn(record);
            });
          }
...
DBReffedArray.prototype.build = function (attrs) {
 ...
 DBReffedArray.prototype.create = function (attrs) {
  attrs[this.parentReferredAs] = this.parent;
  var self = this;
  new this.memberType(attrs).save( function (errors, member) {
    if (errors) throw new Error("Something went wrong"); // TODO Do something better with the errors
...
DBReffedArray.prototype._fetch = function (fn) {
  ...
  conditions[this.parentReferredAs] = {'$ref': this.parent._schema._collection, '$id': this.parent._.doc._id};
